        ...
        for (size_t j = i + 1; j < colliders.size(); ++j)
        {
            ...
            else
            {
                ...
                if (PhysicComponent* physicComponent = colliderA.entity->getComponent<PhysicComponent>())
                {
                    ...
                    const int bestMatchIdx = FindBestMatch(contact, physicComponent->mContacts);
                    if (0 <= bestMatchIdx)
                    {
                        physicComponent->mContacts[bestMatchIdx] = contact;
                    }
                    else
                    {
                        if (3 < physicComponent->mContacts.size())
                        {
                            std::sort(physicComponent->mContacts.begin(), physicComponent->mContacts.end(), sortContact);
                            const float currentArea = triangleAreaEstimate(physicComponent->mContacts[1].position, physicComponent->mContacts[2].position, physicComponent->mContacts[3].position);
                            int bestIdx = -1;
                            for (int cIdx = 1; cIdx <= 3; ++cIdx)
                            {
                                const float area = triangleAreaEstimate(1 == cIdx ? contact.position : physicComponent->mContacts[1].position, 2 == cIdx ? contact.position : physicComponent->mContacts[2].position, 3 == cIdx ? contact.position : physicComponent->mContacts[3].position);
                                if (currentArea < area)
                                {
                                    bestIdx = cIdx;
                                }
                                if (0 < bestIdx)
                                {
                                    physicComponent->mContacts[bestIdx] = contact;
                                }
                            }
                        }
                        else
